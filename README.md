...
3. Сканируйте товары сканером
4. Номера ячеек **автоматически** обнаруживаются и отправляются на печать
5. Принтер печатает этикетки с номерами ячеек и QR-кодами
...
- **`ИНСТРУКЦИЯ_WINDOWS.md`** - полная пошаговая инструкция по установке и запуску на Windows
